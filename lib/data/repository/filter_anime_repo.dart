import 'package:animeto/data/models/anime_model.dart';
import 'package:animeto/data/web_services/dio_web_services.dart';

class AnimeFilterRepo {
  final DioApi dioApi;

  AnimeFilterRepo({required this.dioApi});
  Future<List<AnimeModel>> animeFilter(
      {required int searchPage, required String filter}) async {
    final List<dynamic> searchedAnimes =
        await dioApi.get("anime?page=$searchPage&limit=25&q=&$filter");
    return searchedAnimes.map((anime) => AnimeModel.fromJson(anime)).toList();
  }
}
